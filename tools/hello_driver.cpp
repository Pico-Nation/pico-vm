...
void picoio_assert(bool test, const char* msg) {
   if (!test) {
...
/**
 * Simple implementation of an interpreter using pico-vm.
 */
int main(int argc, char** argv) {
   ...
   using backend_t = picoio::vm::backend<example_host_methods>;
   using rhf_t     = picoio::vm::registered_host_functions<example_host_methods>;

   // register print_num
   rhf_t::add<nullptr_t, &print_num, wasm_allocator>("env", "print_num");
   // register picoio_assert
   rhf_t::add<nullptr_t, &picoio_assert, wasm_allocator>("env", "picoio_assert");
   // register print_name
   ...
   } catch (...) { std::cerr << "pico-vm interpreter error\n"; }
   return 0;
}
