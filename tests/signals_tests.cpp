...
TEST_CASE("Testing signals", "[invoke_with_signal_handler]") {
   bool okay = false;
   try {
      picoio::vm::invoke_with_signal_handler([]() {
         std::raise(SIGSEGV);
      }, [](int sig) {
...
TEST_CASE("Testing throw", "[signal_handler_throw]") {
   CHECK_THROWS_AS(picoio::vm::invoke_with_signal_handler([](){
      picoio::vm::throw_<picoio::vm::wasm_exit_exception>( "Exiting" );
   }, [](int){}), picoio::vm::wasm_exit_exception);
}
...
TEST_CASE("Test signal handler forwarding", "[signal_handler_forward]") {
   ...
   auto guard = picoio::vm::scope_guard{[]{
      std::signal(SIGSEGV, SIG_DFL);
      std::signal(SIGBUS, SIG_DFL);
      std::signal(SIGFPE, SIG_DFL);
      picoio::vm::setup_signal_handler_impl(); // This is normally only called once
   }};
   {
      std::signal(SIGSEGV, &handle_signal);
      std::signal(SIGBUS, &handle_signal);
      std::signal(SIGFPE, &handle_signal);
      picoio::vm::setup_signal_handler_impl();
      sig_handled = 0;
      std::raise(SIGSEGV);
