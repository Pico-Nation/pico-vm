...
BACKEND_TEST_CASE("test reentry", "[reentry]") {
   ...
   struct test_runner {
      picoio::vm::backend<test_runner, TestType>& bkend;
      ...
      void picoio_assert(uint32_t, uint32_t) {}
      void* memset(void*, int, int) { return 0; }
   };

   using rhf_t     = picoio::vm::registered_host_functions<test_runner>;
   using backend_t = picoio::vm::backend<test_runner, TestType>;
   wasm_allocator wa;
   ...
   rhf_t::template add<test_runner, &test_runner::picoio_assert, wasm_allocator>("env", "picoio_assert");
