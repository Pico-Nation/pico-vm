...
#define AUTO_PARAM_WORKAROUND(X) make_dependent<decltype(X)>(X)
   ...
   namespace detail {
      ...
      void validate_ptr( const void* ptr, uint32_t len ) {
         PICO_VM_ASSERT( len <= std::numeric_limits<std::uint32_t>::max() / (uint32_t)sizeof(T), wasm_interpreter_exception, "length will overflow" );
   ...
   struct aligned_array_wrapper {
      ...
      constexpr operator picoio::chain::array_ptr<T>() const {
        return picoio::chain::array_ptr<T>{static_cast<T*>(*this)};
      }
   ...
   struct linear_memory_access {
      ...
      template<typename T>
      void validate_ptr(const void* ptr, uint32_t len) {
         picoio::vm::detail::validate_ptr<T>(ptr, len);
      }
      void validate_c_str(const void* ptr) {
         picoio::vm::detail::validate_c_str(ptr);
      }
   ...
   namespace detail {
      ...
      Converter&& init_wasm_type_converter(Converter&& c, WAlloc* walloc) {
         ::picoio::vm::detail::maybe_add_linear_memory_access(&c, walloc);
   ...
   struct registered_host_functions {
      ...
      static void resolve(Module& mod) {
         ...
         for (int i = 0; i < mod.imports.size(); i++) {
            ...
            PICO_VM_ASSERT(current_mappings.named_mapping.count({ mod_name, fn_name }), wasm_link_exception,
